...
NVIC_EN0_INT19		EQU 0x00080000 ; [x000(12 bit)] 0100 [x0000(16 bit)]Interrupt 19 enable
NVIC_EN0			EQU 0xE000E100 ; IRQ 0 to 31 Set Enable Register
NVIC_PRI4			EQU 0xE000E410 ; IRQ 16 to 19 Priority Register bit [31-30-29 for int19]
...
TIMER0_TAILR		EQU 0x40030028 ; Timer interval : Period
TIMER0_TAPR			EQU 0x40030038 ; PreScale
TIMER0_TAR			EQU	0x40030048 ; Timer register : Current value of the timer
;TIMER0_MATCHR		EQU 0x40030030 ; Match Register Value (set lower 16 bit only)
...
;---------------------------------------------------
LOW					EQU	0x00000014; x32 half period 1KHz 0x00000100
HIGH				EQU	0x00000050; dec total 100
;---------------------------------------------------
...
My_Timer0A_Handler	PROC
				CMP	R5,R6
				BHI	down
				LDR R3, =GPIO_PORTF_DATA ; set data of PF2
				LDR R4, [R3]
				...
				CMP	R2,R0
				BEQ	goHigh
				STR R0, [R1]	; set reload value for Low level
				BFC	R4,#2,#1
				STR	R4,	[R3]
				MOV32 R5,#0xc3500;30d40;13880;4e20;2710
				B	done
goHigh			LDR	R2,=HIGH
				STR R2, [R1]
				ORR	R4,R4,#4
				STR	R4,	[R3]
				MOV32 R5,#0x61a80;;30d40;;#0x4e20;;x2710;0xf4240
				B	done
down			SUB	R5,R5,#1
done
				BX 	LR 
